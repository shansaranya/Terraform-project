...
resource "aws_nat_gateway" "main" {
  count         = var.environment == "prod" ? length(var.private_subnets) : 0
...
resource "aws_route_table" "private" {
  ...
  tags = {
    Name        = "${var.name}-routing-table-private-${format("%03d", count.index+1)}-${var.environment}"
    Environment = var.environment
  }
}

resource "aws_route" "private" {
  count                  = var.environment == "prod" ? length(compact(var.private_subnets)) : 0
